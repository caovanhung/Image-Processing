...
class Ui_detecton_clor_Dialog(object):
    def setupUi(self, detecton_clor_Dialog):
       ...
       # self.tableWidget.setHorizontalHeader (QTableWidgetItem("G"),QTableWidgetItem("G"),QTableWidgetItem("G"))
        ...
        self.xulyanh_pushButton.clicked.connect(self.xyly_GRB)
    ...
    def xyly_GRB(self):
        self.img=self.img[self.img_label.x0:self.img_label.x1,self.img_label.y0:self.img_label.y1]
        self.img=xulyanh(self.img);
        file_img=self.ten+"cat"+".jpg"        
        cv2.imwrite(file_img,self.img.getdata())
        self.GRB_trungbinh=self.img.GRB()
        self.HSV_trungbinh=self.img.HSV()
    ...
    # def GRB_HSV(self):
      #   self.GRB_trungbinh=self.img.GRB()
      #   self.HSV_trungbinh=self.img.GRB()
