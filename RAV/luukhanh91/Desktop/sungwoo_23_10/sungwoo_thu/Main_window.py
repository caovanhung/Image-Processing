 ...
 #Mouse movement events
     ...
     def paintEvent(self, event):
       ...
       # print(self.x1)
        ...
        #  if self.dk==1:           
          #        self.painter.drawRect(self.rect) 
          #        self.painter.drawRect(self.rect1)
                #  painter.drawRect(rect2)
         # if self.dk==2:           
              #    self.painter.drawRect(self.rect) 
              #    self.painter.drawRect(self.rect1)
               #   self.painter.drawRect(self.rect2)                  
class Ui_Detection_edges_Dialog(object):
   ...
   #     if (data==1):
    ...
    def retranslateUi(self, Detection_edges_Dialog):
        ...
        self.comboBox_Baudrate.setItemText(0, _translate("Detection_edges_Dialog", "9600"))
        self.comboBox_Baudrate.setItemText(1, _translate("Detection_edges_Dialog", "19200"))
        self.comboBox_Baudrate.setItemText(2, _translate("Detection_edges_Dialog", "38400"))
        self.comboBox_Baudrate.setItemText(3, _translate("Detection_edges_Dialog", "57600"))
        self.comboBox_Baudrate.setItemText(4, _translate("Detection_edges_Dialog", "115200"))
    ...
    def Detection_edges_open_img(self):
      ...
      #  if self.img_detection_edges_label.get_dem()==1:
            # self.img_detection_edges_label.setfdk(2)
            # print("....",self.img_detection_edges_label.getdk())
        #if self.img_detection_edges_label.get_dem()==2:
             
           #  self.img_detection_edges_label.setfdk(3)
       #      print("....",self.img_detection_edges_label.getdk())
...
#         self.img_xuly=cv2.imread(f_anh_sasds,1)
    ...
    def on_buttonConnect_clicked(self):
        name = self.Port_comboBox.currentText()
        port = self._ports[name]
        self._serial.setPortName(port.systemLocation())
        self._serial.setBaudRate(  
            getattr(QSerialPort, 'Baud' + self.comboBox_Baudrate.currentText()))

        self._serial.setParity(  # QSerialPort::NoParity
            getattr(QSerialPort,'No' + 'Parity'))
        # 设置数据位
        self._serial.setDataBits(  # QSerialPort::Data8
            getattr(QSerialPort, 'Data' + '8'))
        # 设置停止位
        self._serial.setStopBits(  # QSerialPort::Data8
            getattr(QSerialPort, 'OneStop'))
    ...
    def off_buttonDisconnect_clicked(self):
         self._serial.close()
    ...
    def onReadyRead(self):
        if self._serial.bytesAvailable():
            if len(self.text)!=0:
               self._serial.write(self.text)

            data = self._serial.readAll()
            data = data.data()
            data=data.decode('gb2312') 
            print("..............",data)
        #    self.Postion_1_textEdit.setText(str(data))            
